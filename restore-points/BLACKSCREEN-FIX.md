...
- Boot completes but screen stays black after SDDM
- Black screen after login / after `hyprctl reload`
- TTY works (`Ctrl+Alt+F2`), but GUI doesn't start
- Screen stays black only on one monitor / external display
- Laptop screen black but external monitor works (or vice versa)
...
# Systemd user logs
journalctl -b --user -u hyprland --no-pager | tail -50
...
Ensure `/home/kdx/.config/hypr/nvidia.conf` exists:
```ini
...
**Fix 3 — Disable Wayland on NVIDIA (fallback):**

If Hyprland + NVIDIA still fails, use X11 with a different compositor, or try:
```bash
# In /etc/gdm/custom.conf or /etc/sddm.conf.d/
